...
public class WeewxDeviceFactory implements DeviceFactory {
  private static final Logger log = LoggerFactory.getLogger(WeewxDeviceFactory.class);

  @Override
  public boolean supportsDeviceType(DeviceType type) {
    return stream(WeewxDeviceType.values()).anyMatch(t -> t == type);
  }

  @Override
  public Set<? extends DeviceType> getSupportedDeviceTypes() {
    return Set.of(WeewxDeviceType.values());
  }
  ...
  public Optional<Device> createDevice(DeviceType type, String id, String displayName, Map<String, String> parameters, Map<String, String> customIdentifiers) {
    if (supportsDeviceType(type)) {
      ...
      return Optional.of(new WeewxDevice("weather", displayName, customIdentifiers));
    }
